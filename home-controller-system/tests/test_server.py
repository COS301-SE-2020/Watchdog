...
def send_frame_to_server(port):
    ...
    server = NetGear(address='127.0.0.1', port=port, protocol='tcp', pattern=1, **options)
    # while True:
    try:
        (grabbed, frame) = stream.read()
        if not grabbed:
            raise Exception("Could not send video frame image. Please check that your video path is correct!")
        server.send(frame)
